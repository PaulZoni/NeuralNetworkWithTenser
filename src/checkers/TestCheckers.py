...
print('ss')
import src.checkers.TrainCheckersModel as checkers
...
for gen in range(0, 150):
    for game in range(0, 200):
        ...
        while True:
            ...
            if winner == 1:
                win = win + 1
                temp_labels = meta_model.predict_on_batch(temp_data[1:]) + (
                            np.reshape(np.linspace(0.01, 0.1, temp_data.shape[0] - 1), (temp_data.shape[0] - 1, 1)) * (
                                1. / turn))
                ...
                if winner == - 1:
                    ...
                    draw = draw + 1
                temp_labels = meta_model.predict_on_batch(temp_data[1:]) - (
                            np.reshape(np.linspace(0.0001, 0.001, temp_data.shape[0] - 1),
                                       (temp_data.shape[0] - 1, 1)) * (201 - turn))
                data = np.vstack((data, temp_data[1:]))
                labels = np.vstack((labels, temp_labels))
                break
            turn = turn + 1
